  ...
  try {
    ...
    if (testCase.expected) {
      ...
      if (expectedMatch) {
        ...
        console.log(
          `⚠️  EXPECTED VALUE MISMATCH: Got "${analyzeResult.value}", Expected "${testCase.expected}"`
        );
      }
    } else {
...
const totalStructureErrors =
  basicResults.summary.structure_error + edgeResults.summary.structure_error;
const totalRuntimeErrors = basicResults.summary.runtime_error + edgeResults.summary.runtime_error;
const totalExpectedMismatches =
  basicResults.summary.expected_mismatch + edgeResults.summary.expected_mismatch;
...
console.log(
  `❌ Parse Errors: ${totalParseErrors} (${((totalParseErrors / totalTests) * 100).toFixed(1)}%)`
);
console.log(
  `⚠️  Analyze Errors: ${totalAnalyzeErrors} (${((totalAnalyzeErrors / totalTests) * 100).toFixed(1)}%)`
);
console.log(
  `🔧 Structure Errors: ${totalStructureErrors} (${((totalStructureErrors / totalTests) * 100).toFixed(1)}%)`
);
console.log(
  `💥 Runtime Errors: ${totalRuntimeErrors} (${((totalRuntimeErrors / totalTests) * 100).toFixed(1)}%)`
);
console.log(
  `📊 Expected Mismatches: ${totalExpectedMismatches} (${((totalExpectedMismatches / totalTests) * 100).toFixed(1)}%)`
);
