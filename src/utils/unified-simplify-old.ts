...
import { enhancedExponentialSimplification } from './simplify/exponential';
import {
  gcd as gcd_simplify,
  reduceFraction,
...
export interface SimplifyOptions extends MiddleSimplifyOptions {
  /** Apply advanced factorization patterns (default: true) */
  usePatternRecognition?: boolean;
  /** Factor expressions using advanced algorithms (default: true) */
  factor?: boolean;
}
 ...
 * Uses middle-simplify as base and adds factorization capabilities
 */
export function simplify(node: ASTNode, options: SimplifyOptions = {}): ASTNode {
  ...
  try {
    ...
    // Step 1: Apply middle-level simplification first
